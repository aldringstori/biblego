...
const GoNavDrawer = class {
    constructor(hostRef) {
        ...
        this.openEvent = index.createEvent(this, "open", 7);
        this.closeEvent = index.createEvent(this, "close", 7);
        this.navItemClickEvent = index.createEvent(this, "navItemClick", 7);
     ...
     */
    async init(newItems) {
        this.navItems = helper.parseJsonProp(newItems);
    }
    async open() {
        this.active = true;
        this.openEvent.emit();
    ...
    componentWillLoad() {
        this.inheritedAttrs = helper.inheritAttributes(this.el, ['class', 'style', 'items', 'active', 'position'], false);
        try {
            this.navItems = helper.parseJsonProp(this.items);
        }
        catch (e) {
    ...
    openSubMenu(e) {
        ...
        dom.trapFocus(menuItem.querySelector('.nav-menu'));
    ...
    renderNavItems(items, parentItem) {
        ...
        return (index.h("div", { class: { 'nav-menu': true, 'is-sub-nav': isSubNav } }, index.h("div", { class: "nav-drawer-header" }, index.h("div", { class: "header-row" }, parentItem ? (index.h("go-button", { class: "back-btn", flat: true, stack: true, variant: "text", compact: true, onClick: () => this.closeCurrentSubMenu() }, index.h("svg", { slot: "prefix", "aria-hidden": "true", xmlns: "http://www.w3.org/2000/svg", fill: "none", stroke: "currentColor", "stroke-width": "2", "stroke-linecap": "round", "stroke-linejoin": "round", viewBox: "0 0 24 24" }, index.h("path", { d: "M19 12H5M12 19l-7-7 7-7" })), index.h("span", null, "Back"))) : (index.h("span", null)), !isSubNav && index.h("div", { class: "title" }, this.label), index.h("go-button", { class: "close-btn", flat: true, stack: true, variant: "text", compact: true, onClick: () => this.close() }, index.h("svg", { "aria-hidden": "true", xmlns: "http://www.w3.org/2000/svg", fill: "none", stroke: "currentColor", "stroke-width": "2", "stroke-linecap": "round", "stroke-linejoin": "round", viewBox: "0 0 24 24" }, index.h("path", { d: "M18 6 6 18M6 6l12 12" })), index.h("span", null, "Close")))), (items === null || items === void 0 ? void 0 : items.length) > 0 ? (index.h("nav", { "aria-label": isSubNav ? parentItem.label : this.label }, isSubNav && parentItem.url ? (index.h("div", { class: "parent-link" }, index.h("go-link", Object.assign({ href: parentItem.url }, parentItem.linkAttrs, { onClick: (e) => this.handleNavItemClick(e, parentItem) }), index.h("span", { class: "nav-item-label" }, navHelpers.renderIcon(parentItem.icon), index.h("span", null, parentItem.label)), index.h("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", stroke: "currentColor", "stroke-width": "2", "stroke-linecap": "round", "stroke-linejoin": "round", viewBox: "0 0 24 24" }, index.h("path", { d: "M5 12h14M12 5l7 7-7 7" }))))) : null, index.h("ul", null, items.map((item) => this.renderNavItem(item))))) : null));
    }
    renderNavItem(item) {
        var _a;
        let Tag = 'a';
        ...
        return (index.h("li", { class: { 'nav-item': true, 'has-children': hasChildren, 'current': item.isCurrent } }, index.h(Tag, Object.assign({ class: "nav-item-inner" }, attrs), index.h("span", { class: "nav-item-label" }, navHelpers.renderIcon(item.icon), index.h("span", null, item.label)), hasChildren ? (index.h("svg", { class: "children-indicator", fill: "none", stroke: "currentColor", "stroke-width": "2", "stroke-linecap": "round", "stroke-linejoin": "round", viewBox: "0 0 24 24" }, index.h("path", { d: "m9 18 6-6-6-6" }))) : null), item.children ? this.renderNavItems(item.children, item) : null));
    }
    render() {
        let { navItems, active, position, inheritedAttrs } = this;
        return (index.h("go-overlay", Object.assign({ key: '6af4802f093470ef5170198571a1fb1c0c873f86', active: active }, inheritedAttrs, { onOverlayClose: () => this.close() }), index.h("div", { key: '0444ca13a9d55c0d823b76569b68f3d904899b2c', class: { 'nav-drawer': true, 'open': active, [position]: !!position } }, navItems ? index.h("div", { class: "nav-container" }, this.renderNavItems(navItems)) : index.h("slot", null))));
    }
...
exports.go_nav_drawer = GoNavDrawer;

//# sourceMappingURL=go-nav-drawer.cjs.entry.js.map