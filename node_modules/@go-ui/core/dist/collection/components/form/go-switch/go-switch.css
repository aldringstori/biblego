...
go-switch {
  ...
  */
  --switch-text-size: max(calc(var(--switch-handle-size) / 2), 0.875rem);
  display: inline-flex;
  flex-direction: row;
  align-items: center;
...
go-switch.full-width {
  display: flex;
  justify-content: space-between;
}
go-switch.full-width label {
  flex-grow: 1;
}
go-switch label {
  cursor: pointer;
...
go-switch .switch-container {
  position: relative;
}
go-switch .switch-group {
  display: flex;
  align-items: center;
  gap: calc(var(--switch-label-gap) / 2);
}
go-switch .switch-track {
  transition: background-color var(--go-duration-normal) var(--go-timing-function), outline-offset var(--go-duration-fast) var(--go-timing-function);
  display: block;
  position: relative;
...
go-switch .switch-track .switch-text {
  position: absolute;
  top: 50%;
  left: auto;
  right: 20%;
...
go-switch .switch-handle {
  ...
  transition: transform var(--go-duration-fast) var(--go-timing-function);
}
@media (prefers-reduced-motion: reduce) {
  go-switch .switch-handle {
    transition-duration: 0s;
...
go-switch input:checked ~ .switch-group .switch-track {
  --switch-bg-color: var(--switch-bg-color-checked);
}
go-switch input:checked ~ .switch-group .switch-track .switch-handle {
  transform: translateX(calc(var(--switch-width) - var(--switch-handle-size) - 2 * var(--switch-handle-gap)));
}
go-switch input:checked ~ .switch-group .switch-track .switch-text {
  left: 20%;
  right: auto;
}
go-switch input:focus-visible:active ~ .switch-group .switch-track {
  outline-offset: 0;
}
go-switch input:focus-visible ~ .switch-group .switch-track {
